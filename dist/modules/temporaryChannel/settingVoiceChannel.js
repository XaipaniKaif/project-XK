...
export async function settingVoiceChannel(interaction) {
    if (interaction.customId !== 'settingTemporaryVoiceChannel')
        return;
    ...
    async function editNameTempChannel(interaction, channelGet) {
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
            await interaction.showModal(editTemporaryChannel.editNameTemporaryNameChannel).catch((err) => { console.error(err); });
    ...
    ;
    async function editLimitTempChannel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
            await interaction.showModal(editTemporaryChannel.editlimitTemporaryNameChannel);
            const modalSumbit = await interaction.awaitModalSubmit({ time: 60_000 }).catch((err) => { console.error(err); });
            ...
            if (isNaN(newLimit)) {
                ...
                await channel.setUserLimit(newLimit).catch((err) => { console.error(err); });
    ...
    ;
    async function openTempCahnnel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
    ...
    ;
    async function closeTempChannel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
    ...
    ;
    async function addBlacklistTempChannel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
            ...
            const collector = reply.createMessageComponentCollector({ componentType: ComponentType.UserSelect, time: 60_000 })
                ...
                const usersBlack = (await Promise.allSettled(idsUser.map(u => i.guild?.members.fetch(u)))).filter(res => res.status === 'fulfilled').map(res => res.value);
    ...
    ;
    async function addWhitelistTempChannel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
            ...
            const collector = reply.createMessageComponentCollector({ componentType: ComponentType.UserSelect, time: 60_000 })
                ...
                const idsUser = i.values;
                const users = (await Promise.allSettled(idsUser.map(async (u) => await i.guild?.members.fetch(u)))).filter((filter) => filter.status === 'fulfilled').map((result) => result.value);
    ...
    ;
    async function setOnNFWS(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
    ...
    ;
    async function setOffNFWS(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
    ...
    ;
    async function banUserTempChannel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
    ...
    ;
    async function muteUserInTempChannel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
    ...
    async function setAdminChannel(interaction, channelGet) {
        if (!interaction.guild)
            return;
        const channel = await interaction.guild?.channels.fetch(channelGet.get(interaction.user.id));
        if (channel instanceof VoiceChannel) {
