...
const channelOBJ:Map<string, string> = new Map();


export default {
    createVoiceTemporaryChannel: async function(oldState: VoiceState, newState: VoiceState) {
        ...
        if (newState.channelId === channelCreateID) {
            ...
            if (channelOBJ.has(newState.member.id)) {
                const key = channelOBJ.get(newState.member.id) as string
                const channelGet = await newState.guild.channels.fetch(key) as VoiceChannel
        ...
        if (oldState.channelId !== newState.channelId && oldState.channelId === channelOBJ.get(oldState.member?.id || '') && oldState.channelId !== channelCreateID) {
            ...
            const channel = channelOBJ.get(oldState.member?.id) as string
